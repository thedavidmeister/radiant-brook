...
class DefaultController extends Controller
{
    ...
    public function tradeIndex(Request $request)
    {
        ...
        '-Diff-' => '',
        'BTC Profit' => $tp->profitBTC()->getAmount(),
        'BTC Profit USD value (midpoint)' => $tp->profitBTC()->getAmount() * $tp->midprice(),
        'USD Profit' => $tp->profitUSD()->getAmount(),
        'Is profitable' => $tp->isProfitable() ? 'Yes' : 'No',
        'Has dupes' => $tp->hasDupes() ? 'Yes' : 'No',
        'Is valid trade' => $tp->isValid() ? 'Yes' : 'No',
        '-Dupes-' => '',
        'Dupe bid range' => $tp->bidPrice()->getAmount() * $tp::DUPE_RANGE_MULTIPLIER,
        'Dupe bids' => var_export($tp->dupes()['bids'], true),
        'Dupe ask range' => $tp->askPrice()->getAmount() * $tp::DUPE_RANGE_MULTIPLIER,
